...
function parseRepoAndPR(input: {
  ...
  if (input.prUrl) {
    // Strip trailing slashes, hashes, and query parameters
    const clean = input.prUrl.trim().replace(/[?#].*$/, '').replace(/\/+$/, '');

    // Match: https://github.com/owner/repo/pull/123 (or /pull/123/files, /pulls/123, etc.)
    const urlMatch = clean.match(/github\.com\/([^/]+)\/([^/]+)\/pulls?\/(\d+)/i) ||
      clean.match(/^([^/#\s]+)\/([^/#\s]+)#(\d+)$/) ||
      clean.match(/^([^/#\s]+)\/([^/#\s]+)\/pulls?\/(\d+)/i);

    if (urlMatch) {
      return {
        owner: urlMatch[1].trim(),
        repo: urlMatch[2].trim().replace(/\.git$/, ''),
        pullNumber: parseInt(urlMatch[3].trim(), 10),
      };
    }
  }

  let owner = input.owner ? input.owner.trim() : '';
  let repo = input.repo ? input.repo.trim() : '';
  const pullNumber = input.pullNumber ? parseInt(String(input.pullNumber).trim(), 10) : 0;
...
// Organization Repositories Explorer Endpoints
apiRouter.post('/github/org-repos', async (req: Request, res: Response) => {
  try {
    const { token, org } = req.body;
    const targetOrg = org || 'angular';

    const result = await octokitService.listOrgRepos(token, targetOrg);
    res.json(result);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ success: false, error: msg });
...
apiRouter.post('/github/user-orgs', async (req: Request, res: Response) => {
  try {
    const { token } = req.body;
    if (!token) {
      res.status(400).json({ success: false, error: 'Token is required to list user organizations.' });
      return;
    }

    const result = await octokitService.listUserOrgs(token);
    res.json(result);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ success: false, error: msg });
...
apiRouter.post('/github/repo-labels', async (req: Request, res: Response) => {
  try {
    ...
    const result = await octokitService.listRepoLabels(token, targetOwner, targetRepo);
    res.json(result);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ success: false, error: msg });
...
apiRouter.post('/github/repo-milestones', async (req: Request, res: Response) => {
  try {
    const { token, owner, repo, state } = req.body;
    let targetOwner = owner;
    let targetRepo = repo;
    ...
    const result = await octokitService.listRepoMilestones(token, targetOwner, targetRepo, state || 'all');
    res.json(result);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(500).json({ success: false, error: msg });
...
apiRouter.post('/github/repo-prs-filtered', async (req: Request, res: Response) => {
  try {
    const { token, owner, repo, state, label, milestone } = req.body;
    let targetOwner = owner;
    let targetRepo = repo;
    ...
    const result = await octokitService.listPullRequestsWithFilters(token, targetOwner, targetRepo, {
      state: state || 'open',
      label,
      milestone,
    });
...
// ==========================================
// COVERITY STREAMS & DEFECTS ENDPOINTS
// ==========================================
apiRouter.get('/coverity/streams', (_req: Request, res: Response) => {
  const streams = securityScanService.getCoverityStreams();
  res.json({
    success: true,
...
apiRouter.get('/coverity/streams/:id', (req: Request, res: Response) => {
  const id = String(req.params['id'] || '');
  const stream = securityScanService.getCoverityStream(id);
  if (!stream) {
    res.status(404).json({ error: `Coverity stream '${id}' not found` });
    return;
  }
  res.json({ success: true, stream });
});

apiRouter.post('/coverity/streams', (req: Request, res: Response) => {
  const { name, project, defects, branch } = req.body;
  if (!name) {
    res.status(400).json({ error: 'Stream name is required' });
    return;
  }
  const streamDefects = Array.isArray(defects) ? defects : [];
  const highCount = streamDefects.filter((d: { impact?: string }) => d.impact === 'High').length;
  const medCount = streamDefects.filter((d: { impact?: string }) => d.impact === 'Medium').length;
  const lowCount = streamDefects.filter((d: { impact?: string }) => d.impact === 'Low').length;

  const newStream = securityScanService.addCoverityStream({
    id: (req.body.id || name.toLowerCase().replace(/[^a-z0-9-]/g, '-')).trim(),
    name: name.trim(),
    project: project || 'custom/project',
    branch: branch || 'main',
    lastAnalyzed: new Date().toISOString(),
    totalDefects: streamDefects.length,
    highImpactCount: highCount,
    mediumImpactCount: medCount,
    lowImpactCount: lowCount,
    defects: streamDefects,
  });

  res.status(201).json({ success: true, stream: newStream });
});

apiRouter.patch('/coverity/streams/:streamId/defects/:cid', (req: Request, res: Response) => {
  const streamId = String(req.params['streamId'] || '');
  const cid = String(req.params['cid'] || '');
  const { status } = req.body;
  const updated = securityScanService.updateCoverityDefectStatus(streamId, Number(cid), status);
  if (!updated) {
    res.status(404).json({ error: 'Defect or stream not found' });
    return;
  }
...
// ==========================================
// APPSCAN REPORTS & FINDINGS ENDPOINTS
// ==========================================
apiRouter.get('/appscan/reports', (_req: Request, res: Response) => {
  const reports = securityScanService.getAppScanReports();
  res.json({
    success: true,
...
apiRouter.get('/appscan/reports/:id', (req: Request, res: Response) => {
  const id = String(req.params['id'] || '');
  const report = securityScanService.getAppScanReport(id);
  if (!report) {
    res.status(404).json({ error: `AppScan report '${id}' not found` });
    return;
  }
  res.json({ success: true, report });
});

apiRouter.post('/appscan/reports', (req: Request, res: Response) => {
  const { scanName, scanType, targetApplication, findings } = req.body;
  if (!scanName) {
    res.status(400).json({ error: 'Scan name is required' });
    return;
  }
  const reportFindings = Array.isArray(findings) ? findings : [];
  const highCount = reportFindings.filter((f: { severity?: string }) => f.severity === 'High').length;
  const medCount = reportFindings.filter((f: { severity?: string }) => f.severity === 'Medium').length;
  const lowCount = reportFindings.filter((f: { severity?: string }) => f.severity === 'Low').length;
  const infoCount = reportFindings.filter((f: { severity?: string }) => f.severity === 'Information').length;

  const newReport = securityScanService.addAppScanReport({
    id: (req.body.id || scanName.toLowerCase().replace(/[^a-z0-9-]/g, '-')).trim(),
    scanName: scanName.trim(),
    scanType: scanType || 'SAST',
    timestamp: new Date().toISOString(),
    targetApplication: targetApplication || 'enterprise-application',
    highCount,
...
apiRouter.patch('/appscan/reports/:reportId/findings/:findingId', (req: Request, res: Response) => {
  const reportId = String(req.params['reportId'] || '');
  const findingId = String(req.params['findingId'] || '');
  const { status } = req.body;
  const updated = securityScanService.updateAppScanFindingStatus(reportId, findingId, status);
  if (!updated) {
    res.status(404).json({ error: 'Finding or report not found' });
    return;
  }
...
// ==========================================
// SECURITY AI REMEDIATION & FIX ENDPOINTS
// ==========================================
apiRouter.post(['/security/fix', '/security/remediate'], async (req: Request, res: Response) => {
  try {
    const { source, defectId, checkerOrCwe, title, file, vulnerableCode, existingRemediation, contextDiff } = req.body;

    if (!vulnerableCode && !existingRemediation) {
      res.status(400).json({ error: 'Vulnerable code snippet or existing remediation is required' });
      return;
    }

    const fixResult = await securityScanService.generateSecurityFix({
      source: source || 'custom',
      defectId: defectId || 'DEFECT-1',
      checkerOrCwe: checkerOrCwe || 'SECURITY-DEFECT',
      title: title || 'Security Defect Fix',
      file: file || 'src/main.ts',
      vulnerableCode: vulnerableCode || '',
      existingRemediation: existingRemediation || '',
      contextDiff,
    });

    res.json(fixResult);
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : 'Failed to generate security remediation';
    res.status(500).json({ error: msg });
  }
});
